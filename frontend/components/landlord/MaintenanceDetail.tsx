...
import { Calendar, FileText, MessageCircle, Clock, CheckCircle2, XCircle, Loader2, User, Phone, Mail, MapPin, AlertTriangle, Wrench, Image } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { Badge } from '@/components/ui/badge';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { useLandlordMaintenanceDetail, useAddMaintenanceComment, useUpdateMaintenance, useScheduleMaintenance } from '@/lib/query/hooks/use-landlord-maintenance-detail';
import { MaintenanceStatus, MaintenancePriority } from '@/lib/query/hooks/use-landlord-maintenance';
...
export function MaintenanceDetail({ requestId, className = '' }: MaintenanceDetailProps) {
    const { user } = useAuthStore();
    const [newComment, setNewComment] = useState('');
    const [selectedStatus, setSelectedStatus] = useState<MaintenanceStatus | ''>('');
    const [selectedPriority, setSelectedPriority] = useState<MaintenancePriority | ''>('');
    const [scheduleDate, setScheduleDate] = useState('');
    const [scheduleTime, setScheduleTime] = useState('');

    const { data: request, isLoading, error } = useLandlordMaintenanceDetail(requestId);
    ...
    const handleStatusUpdate = () => {
        ...
        updateMutation.mutate({ requestId: request.id, data: { status: selectedStatus } });
        setSelectedStatus('');
    };

    const handlePriorityUpdate = () => {
        if (!selectedPriority || !request || updateMutation.isPending) return;
        updateMutation.mutate({ requestId: request.id, data: { priority: selectedPriority } });
        setSelectedPriority('');
    };

    const handleSchedule = () => {
        if (!scheduleDate || !scheduleTime || !request || scheduleMutation.isPending) return;
        scheduleMutation.mutate({ requestId: request.id, date: scheduleDate, time: scheduleTime });
        setScheduleDate('');
        setScheduleTime('');
    };
        ...
        return (
            <div className={`bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-neutral-100 p-8 ${className}`}>
                <div className="flex items-center justify-center p-12">
                    ...
                    <span className="text-neutral-500">Loading maintenance details...</span>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className={`bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-neutral-100 p-8 ${className}`}>
                <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed border-neutral-200 rounded-3xl">
                    ...
                    <h3 className="text-lg font-semibold text-neutral-900 mb-2">Maintenance request not found</h3>
                    <p className="text-neutral-500 mb-6">The maintenance request details could not be loaded.</p>
                </div>
            </div>
        );
    }
    ...
    const statusConfig = {
        OPEN: { color: 'bg-red-50 text-red-800 border-red-200', icon: AlertTriangle },
        IN_PROGRESS: { color: 'bg-blue-50 text-blue-800 border-blue-200', icon: Clock },
        COMPLETED: { color: 'bg-emerald-50 text-emerald-800 border-emerald-200', icon: CheckCircle2 },
        CANCELLED: { color: 'bg-neutral-50 text-neutral-600 border-neutral-200', icon: XCircle },
    } as const;
        ...
        <div className={`space-y-6 ${className}`}>
                ...
                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                    <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                                <Badge className={`font-semibold ${statusConfig[request.status]?.color || 'bg-neutral-100 text-neutral-800'}`}>
                                    {request.status.replace('_', ' ').toUpperCase()}
                                </Badge>
                                <Badge variant="outline" className={`ml-2 ${priorityConfig[request.priority]?.color}`}>
                            ...
                            <div className="mt-6 p-4 bg-amber-50 rounded-2xl border border-amber-100">
                                <div className="flex items-baseline gap-2">
                                    ...
                                    <span className="text-sm font-medium text-amber-700">
                                        Deadline: {format(new Date(request.deadline), 'MMM d, yyyy • h:mm a')}
                                    </span>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="text-right">
                            ...
                            <div className="font-semibold text-neutral-900">{request.propertyName}</div>
                        </div>
                        <div className="text-right">
                ...
                <div className="bg-white rounded-3xl p-6 border border-neutral-100 shadow-sm">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className="font-semibold text-neutral-900">{request.tenant.name}</div>
                    ...
                    {request.assignedTo ? (
                        <div className="space-y-3">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="font-semibold text-neutral-900">{request.assignedTo.name}</div>
                                    <div className="text-sm text-neutral-500">Maintenance Personnel</div>
                                </div>
                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                            <Select value={selectedStatus} onValueChange={(value) => setSelectedStatus(value as MaintenanceStatus)}>
                            ...
                            <Button onClick={handleStatusUpdate} disabled={!selectedStatus || updateMutation.isPending}>
                    ...
                    {/* Update Priority */}
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-neutral-700">Update Priority</label>
                        <div className="flex gap-2">
                            <Select value={selectedPriority} onValueChange={(value) => setSelectedPriority(value as MaintenancePriority)}>
                                <SelectTrigger className="flex-1">
                            ...
                            <Button onClick={handlePriorityUpdate} disabled={!selectedPriority || updateMutation.isPending}>
                                {updateMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Update'}
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-neutral-700">Schedule Maintenance</label>
                        <div className="flex gap-2">
                            <Input
                                type="date"
                                value={scheduleDate}
                            ...
                            <Input
                                ...
                                className="w-24"
                            />
                            <Button onClick={handleSchedule} disabled={!scheduleDate || !scheduleTime || scheduleMutation.isPending}>
            ...
            {request.photos.length > 0 && (
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {request.photos.map((photo: { id: string; filename: string; url: string; uploadedAt: string }) => (
                            <div key={photo.id} className="group bg-white hover:bg-neutral-50 rounded-2xl p-6 border border-neutral-100 hover:shadow-md transition-all cursor-pointer">
                                <div className="flex items-start gap-3 mb-2">
                                    <FileText size={16} className="text-neutral-400 mt-0.5 flex-shrink-0" />
                                    <div className="flex-1 min-w-0">
                                        <div className="font-medium text-neutral-900 truncate mb-1" title={photo.filename}>{photo.filename}</div>
            ...
            <div>
                ...
                <div className="space-y-4 mb-8">
                    {request.comments.map((comment: { id: string; author: { id: string; name: string; role: string }; content: string; createdAt: string }) => (
                        <div key={comment.id} className="flex gap-4 p-6 bg-white rounded-2xl border border-neutral-100">
                            <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center flex-shrink-0">
                                <span className="font-semibold text-white text-xs">
                                    {comment.author.role.charAt(0).toUpperCase()}
                                </span>
                            </div>
                            <div className="flex-1 min-w-0">
                    ...
                    <form onSubmit={handleSubmitComment} className="p-6 bg-neutral-50 rounded-2xl border-2 border-dashed border-neutral-200">
                        <div className="flex gap-3">
                            <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center flex-shrink-0">
                                <span className="font-semibold text-white text-xs">{user?.firstName?.[0] || 'L'}</span>
                            </div>
